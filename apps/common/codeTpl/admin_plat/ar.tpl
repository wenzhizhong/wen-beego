...
func (ar *{{.ModelName}}Ar) Insert(tx *gorm.DB, data dto.{{.ModelName}}Dto) error {
	if data.Id == "" {
...
func (ar *{{.ModelName}}Ar) Update(tx *gorm.DB, data dto.{{.ModelName}}Dto) error {
	if data.Id == "" {
...
func (ar *{{.ModelName}}Ar) CheckExists(id string) (bool, error) {
	var count int64
	err := global.GetReadDb().
		Model(&models.{{.ModelName}}{}).
		Where("deleted = 0 AND id = ?", id).
