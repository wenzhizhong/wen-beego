 ...
 * @return
 */
func (a *PlatRoleClassifyAr) GetClassifiesByRoleId(unitId string, roleId string) (platRoleClassify []models.PlatRoleClassify, err error) {
	if unitId == "" || roleId == "" {
		str := fmt.Sprintf("GetClassifiesByName():参数不能为空, unitId:%s, roleId:%s", unitId, roleId)
 ...
 * @return
 */
func (a *PlatRoleClassifyAr) GetClassifiesByName(unitId string, classifyName string) (platRoleClassify []models.PlatRoleClassify, err error) {
	if unitId == "" || classifyName == "" {
		str := fmt.Sprintf("GetClassifiesByName():参数不能为空, unitId:%s, classifyName:%s", unitId, classifyName)
		global.Log.Error(str)
		return platRoleClassify, errors.New(str)
	}
	tableClassify := a.TableName()
